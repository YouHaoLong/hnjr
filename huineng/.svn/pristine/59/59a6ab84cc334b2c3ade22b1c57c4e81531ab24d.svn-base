...
function user() {
	...
	$.ajax({
			...
			for(var i = 0; i < allList.length; i++) {
				...
				var html = '';
				html += '<tr><td class=" col-xs-1">'+allList[i].userNum+'</td><td class=" col-xs-1">'+allList[i].name+'</td><td class=" col-xs-1">'+sex+'</td><td class=" col-xs-2">'+allList[i].mobile_one+'</td><td class=" col-xs-1">'+user_type+'</td><td class=" col-xs-1">'+allList[i].employeeName+'</td><td class=" col-xs-2">'+allList[i].create_date+'</td><td class=" col-xs-1"><a class="btn btn1" href="javascript:xmxz('+allList[i].id+')" >选择</a></td></tr>';
...
function choice(index){
	...
	$.ajax({
		...
		success: function(data) {
			...
			$('#projectname').html(data.obj[0].name);
			$('#productcode').html(data.obj[0].grantTime);
			$('#type').html(type);
		},
		error: function() {
			console.log('请求失败')
...
function details(index,productid) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(all.fileUrl==''||all.fileUrl==null){
				var stc = '#';
				$('#fileUrl').html('无附件');
				
			}else{
				var stc='/project/downloadFile?realPath='+all.fileUrl+'';
...
function pmt_deta(index,recordId) {
	...
	$.ajax({
		...
		success: function(data) {
			...
			var all = data.obj;
			if(all.financingfinancingType == 1) {
			...
			} else if(all.financingType == 5) {
				var	financingType = '偏债型';
			} else if(all.financingType == 6) {
				var	financingType = '债券型';
			} else if(all.financingType == 7) {
				var	financingType = '保本型';
			} else if(all.financingType == 8) {
				var	financingType = '指数型';
			} else if(all.financingType == 9) {
			...
			if(all.url==''||all.url==null){
				var stc = '#';
				$('#paymt15').html('无附件');
			}else{
				var stc='/project/downloadFile?realPath='+all.url+'';
				$('#paymt15').attr('href',stc);
				$('#paymt15').html('下载附件');
			}
...
function dzqr(index) {
	...
	$('#queren').modal('show');
	var html = '';
	html += '<button type="button" class="btn" onclick="qued(' + index + ')">是</button><button type="button" class="btn btn" data-dismiss="modal">否</button>';
	$('#delc').append(html);
