	...
	$.ajax({
		...
		success: function(data) {
			...
			for(var i = 0; i < allList.length; i++) {
				...
				var	draw_state='已发放';
				html += '<tr><td scope="row"><input type="checkbox" name="infoID" class="infoID" value="'+allList[i].drawId+'"></td><td>'+allList[i].orderId+'</td><td>'+allList[i].productName+'</td><td>'+allList[i].productCreate+'</td><td>'+allList[i].userName+'</td><td>'+allList[i].mobile_one+'</td><td>'+allList[i].applyMoney+'</td><td>'+state+'</td><td>'+allList[i].endTime+'</td><td>'+allList[i].employeeName+'</td><td>'+allList[i].phone+'</td><td>'+allList[i].draw_money+'</td><td>'+draw_type+'</td><td>'+draw_state+'</td><td>'+allList[i].update_date+'</td><td></td></tr>';
